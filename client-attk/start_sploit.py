...
def _extract_team_addr(team_item):
    for key in ('addr', 'address', 'ip', 'host', 'hostname', 'target'):
        value = team_item.get(key)
...
def normalize_teams(teams):
    if isinstance(teams, dict):
        return {str(name): (None if addr is None else str(addr))
                for name, addr in teams.items()}

    if isinstance(teams, list):
        normalized = {}
        for idx, item in enumerate(teams, start=1):
            if isinstance(item, str):
                normalized[item] = item
                continue

            if isinstance(item, dict):
                team_name = item.get('name') or item.get('team') or item.get('id') or idx
                normalized[str(team_name)] = _extract_team_addr(item)
                continue

            normalized['team_{}'.format(idx)] = None
        return normalized

    raise ValueError('Unsupported TEAMS type in server config: {}'.format(type(teams).__name__))
...
def get_target_teams(args, teams, attack_no):
    ...
    teams_without_addr = [name for name, addr in teams.items() if addr is None]
    if teams_without_addr and attack_no == 1 and not args.not_per_team:
        logging.warning('Some teams do not have an address in TEAMS and will be run without target args: {}'.format(
            ', '.join(sorted(teams_without_addr)[:PRINTED_TEAM_NAMES] + (['...'] if len(teams_without_addr) > PRINTED_TEAM_NAMES else []))))

    if args.distribute is not None:
        k, n = args.distribute
        teams = {name: addr for name, addr in teams.items()
                 if binascii.crc32((addr or name).encode()) % n == k - 1}
